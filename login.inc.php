...
if (!file_exists(DB_FILE)) {
	...
	echo "<h2>Netsim installation</h2>";
	echo "<p>No databases file was found at " . DB_FILE . ", so attempting to create it now...</p>";

	try {
		$db = new SQLite3(DB_FILE);
		$db->exec("CREATE TABLE user (id integer PRIMARY KEY, name text, password text)");
		$db->exec("INSERT INTO user (name, password) VALUES ('erinn', 'erinn')");
